function [similarity, match_point] = MatLabLCSS(a, b, delta, epsilon, gait_cycle_num, showPlots, transpose)
...
if nargin < 6 ,
    disp('similarity = lcsMatching(a, b, delta, epsilon, gait_cycle_num, showPlots, transpose)');
end
if nargin < 7,
    setTranspose = 0;
end
...
%==============================================
if showPlots
    figure('Name', sprintf('Gait Cycle %d : Matching Region and Point Correspondence', gait_cycle_num));    
    ax = subplot(2,1,1);
    ...
    % --------
    subplot(2,1,2);
    title(['Point Correspondence, Similarity _{[\delta=' num2str(delta)  ...
...
U(1) = max(traj(1,[1:window+1])+epsilon);
L(1) = min(traj(1,[1:window+1])-epsilon);
